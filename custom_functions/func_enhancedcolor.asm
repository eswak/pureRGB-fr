SECTION "Enhanced GBC Colors", ROMX

DEF w2MapViewHLPointer EQU $d0fa	;wram bank 2 backup. 2 bytes
DEF w2CurMap EQU $d0fc
...
; pureGREENFRnote: ADDED: entry point that bypasses hAutoBGTransferEnabled check.
; Called from SetPal_Overworld's .enhancedGBCOverworld path where we always want
; to regenerate the full attribute table, even if hAutoBGTransferEnabled is non-zero.
MakeOverworldBGMapAttributes_Force::
	ld a, [wMapViewVRAMPointer]
	ld b, a
	ld a, [wMapViewVRAMPointer+1]
	ld c, a
	jr MakeOverworldBGMapAttributes.jump_in
...
;only do the attributes when walking around, not during a menu or text since that will mess up the settings
	; pureGREENFRnote: ADDED: bypass hAutoBGTransferEnabled check when $FFF6 bit 3 is set.
	; $FFF6 bit 3 indicates this is a row/column update during player movement (set by .updateMapView).
	; During walking, hAutoBGTransferEnabled is 1 but we still need to update the attributes for
	; the new row/column that scrolled into view. Without this bypass, the partial update
	; (MakeOverworldBGMapAttributes_RolColUpdate) would never be reached during walking.
	ld hl, $FFF6
	bit 3, [hl]
	jr nz, .endAutoBGTransferFlag
...
.jump_in	;label used for being called from other functions
	...
	; pureGREENFRnote: FIXED: was incorrectly loading the VALUE from $FFF6 and using it as an address.
	; Must load the ADDRESS $FFF6 into HL and then check bit 3 of the value at that address.
	ld hl, $FFF6
	bit 3, [hl]
...
;only for GBC and only if option is active
	...
	;shinpokerednote: ADDED: ensure rBGP, rOBP0, rOBP1 are initialized with FadePal4 before reading
	;This is needed because BufferAllEnhancedColorsGBC reads from these registers as patterns
	ld hl, FadePal4
	ld a, [hli]
	ldh [rBGP], a
	ld a, [hli]
...
BufferAllEnhancedColorsGBC::
	;shinpokerednote: CHANGED: write directly to wGBCFullPalBuffer in bank 0 (matches shinpokered)
	call BufferAllEnhancedColorsGBC_BGP0to3Loop
	call BufferAllEnhancedColorsGBC_BGP4to7Loop
	call BufferAllEnhancedColorsGBC_OBP0to3Loop
	call BufferAllEnhancedColorsGBC_OBP4to7Loop
	ret

BufferAllEnhancedColorsGBC_BGP0to3Loop::
	ld hl, wGBCFullPalBuffer
	xor a
.loop
	call BufferAllEnhancedColorsGBC_readwriteinc
	cp 16
	jr c, .loop
	ret

BufferAllEnhancedColorsGBC_BGP4to7Loop::
...
BufferAllEnhancedColorsGBC_OBP0to3Loop::
...
BufferAllEnhancedColorsGBC_OBP4to7Loop::
...
BufferAllEnhancedColorsGBC_readwriteinc::
	ld [wGBCColorControl], a
	push de
	push hl
	call BufferAllEnhancedColorsGBC_ReadMasterPals	;get the color into DE
	push bc
	;shinpokerednote: ADDED: check hGBC == 2 before applying gamma correction (matches GBCGamma predef)
	ldh a, [hGBC]
	cp 2
	jr nz, .skipGamma	;don't apply gamma correction if not GBC mode 2
	call GetRGB
	call BufferAllEnhancedColorsGBC_isBlack	;shinpokerednote: ADDED: check if color is black before applying matrix
	jr c, .skipMatrix	;don't apply matrix to black colors
	call MixColorMatrix	;shinpokerednote: ADDED: apply color mixing matrix before gamma conversion
.skipMatrix
	call GammaConv
	call WriteRGB
	jr .done
.skipGamma
	;if not GBC mode 2, just write the color as-is without conversion
.done
	pop bc
	pop hl
	;shinpokerednote: CHANGED: wGBCFullPalBuffer is now in bank 0, write directly
	ld a, d
...
BufferAllEnhancedColorsGBC_isBlack::
	;Check if the current color is a type of black. Return carry if black.
	;check red value
	ld a, [hRGB + 0]
	cp 4	
...
BufferAllEnhancedColorsGBC_ReadMasterPals::
...
;	push af
	...
	; pureGREENFRnote: FIXED: removed CopyGBCFullPalBuffer1to2 and use wGBCFullPalBuffer directly.
	; The copy to w2GBCFullPalBuffer (WRAM bank 2) was broken because GBCBufferFastTransfer_BGP
	; reads via SP trick without switching rSVBK to bank 2, causing it to read garbage data.
	; wGBCFullPalBuffer is in WRAM bank 0 (always accessible), so use it directly like shinpokered.
...
.transfer
	...
	; pureGREENFRnote: FIXED: removed CopyGBCFullPalBuffer1to2 and use wGBCFullPalBuffer directly.
	; Same fix as UpdateEnhancedGBCPal_BGP: w2GBCFullPalBuffer was in WRAM bank 2 but never
	; bank-switched before reading. wGBCFullPalBuffer is in bank 0, always accessible.
