_CeruleanCityRivalPreBattleText::
	text "<RIVAL>: Yo!"
	line "<PLAYER>!"

	para "Alors tu t'en"
	line "sors?"

	para "Moi c'est l'pied!"
	line "J'ai chopé un tas"
	cont "de #MON méga"
	cont "puissants et"
	cont "tétra balèzes!"

	para "Fais-moi voir"
	line "c'que t'as, p'tit"
	cont "minable!"
	done

_CeruleanCityRivalDefeatedText::
	text "Houlà!"
	line "Calmos minable!"
	cont "T'as gagné, OK!"
	prompt

_CeruleanCityRivalVictoryText::
	text "Pfff!"
	line "Une perte de"
	cont "temps. Mi-nable!"
	prompt

_CeruleanCityRivalIWentToBillsText::
	text "<RIVAL>: Hé!"
	line "Tu sais quoi?"

	para "Chuis allé chez"
	line "LEO et y m'a tout"
	cont "montré ses"
	cont "#MON rares!"

	para "Ca m'a rajouté"
	line "plein d'pages sur"
	cont "mon bô #DEX!"

	para "Tout l'monde sait"
	line "que LEO est un"
	cont "#MANIAC!"

	para "Il a même inventé"
	line "le stockage des"
	cont "#MON par PC!"

	para "T'as qu'à aller"
	line "l'voir!"

	para "Bon, je trace."
	line "Tchao minab'!"
	done

_CeruleanCityRocketText::
	text "Hé le lutin!"
	line "C'est pas chez"
	cont "toi, ici..."
	cont "Quoi? Moi?"

	para "J'suis innocent!"
	line "Hein? Quoi dit?"
	cont "Tu m'crois pas?"
	cont "Tu m'cherches?"
	done

_CeruleanCityRocketReceivedTM28Text::
	text "<PLAYER> récupère"
	line "@"
	text_ram wStringBuffer
	text "!@"
	text_end

_CeruleanCityRocketIBetterGetMovingText::
	text_start

	para "J'décampe..."
	line "Bye bye!@"
	text_end

_CeruleanCityRocketTM28NoRoomText::
	text "Tu portes trop"
	line "d'trucs!"

	para "Je dois te donner"
	line "ça avant d'fuir."
	cont "C'est écrit dans"
	cont "l'histoire!"
	done

_CeruleanCityRocketIGiveUpText::
	text "Stop!"
	line "J'me rends!"
	cont "T'es trop fort!"
	prompt

_CeruleanCityRocketIllReturnTheTMText::
	text "OK! Voilà la CT"
	line "qu'j'ai volée!"
	prompt

_CeruleanCityCooltrainerMText::
	text "T'es un dresseur?"
	line "Capturer..."
	cont "Collectionner..."
	cont "Se friter..."
	cont "Dur, hein?"
	done

_CeruleanCitySuperNerd1Text::
	text "Ce buisson devant"
	line "la boutique blo-"
	cont "que le passage."

	para "Il doit y avoir"
	line "un moyen de"
	cont "passer."
	done

_CeruleanCitySuperNerd1_AfterTreeDelete::
	text "Un type fou avec"
	line "des lunettes de"
	cont "soleil est arrivé"
	cont "comme un cyclone!"
	para "Puis son RONFLEX"
	line "a carrément"
	cont "arraché l'arbre"
	cont "qui était devant"
	cont "la boutique!"
	para "Puis ils sont"
	line "partis aussi vite"
	cont "qu'ils étaient"
	cont "arrivés!"
	para "Quel spectacle!"
	done

_CeruleanCitySuperNerd2Text::
	text "Tu fais une"
	line "encyclopédie sur"
	cont "les #MON?"
	cont "Pas mal..."
	done

_CeruleanCityGuardText::
	text "Ces pauvres gens"
	line "ont été volés."

	para "L'odieuse TEAM"
	line "ROCKET est"
	cont "derrière tout ça."
	cont "C'est sûr!"

	para "Même les forces"
	line "de police ont du"
	cont "mal à lutter!"
	done

_CeruleanCityCooltrainerF1SlowbroUseSonicboomText::
	text "Allez! FLAGADOSS!"
	line "Attaque! Attaque!"
	cont "Mais non!!! Tu"
	cont "m'écoutes, oui?"
	done

_CeruleanCityCooltrainerF1SlowbroPunchText::
	text "FLAGADOSS, cogne!"
	line "Mais non! Tu fais"
	cont "n'importe quoi!"
	done

_CeruleanCityCooltrainerF1SlowbroWithdrawText::
	text "FLAGADOSS, REPLI!"
	line "Non! Pas ça!"

	para "Pfff... C'est dur"
	line "de contrôler un"
	cont "#MON!"

	para "Seuls les bons"
	line "dresseurs se font"
	cont "obéir par leurs"
	cont "#MON!"
	done

_CeruleanCitySlowbroTookASnoozeText::
	text "FLAGADOSS pionce"
	line "comme une masse."
	done

_CeruleanCitySlowbroIsLoafingAroundText::
	text "FLAGADOSS"
	line "flâne..."
	done

_CeruleanCitySlowbroTurnedAwayText::
	text "FLAGADOSS se"
	line "balade..."
	done

_CeruleanCitySlowbroIgnoredOrdersText::
	text "FLAGADOSS ignore"
	line "les ordres..."
	done

_CeruleanCityCooltrainerF2Text::
	text "Ze veux une zolie"
	line "BICYCLETTE!"

	para "Ze la veux toute"
	line "rouze!"
	done

_CeruleanCitySuperNerd3Text::
	text "Des #MON trop"
	line "puissants vivent"
	cont "dans la CAVE"
	cont "AZUREE!"

	para "Seul le CHAMPION"
	line "de la LIGUE"
	cont "#MON a le"
	cont "droit d'y entrer!"
	done

_CeruleanCitySignText::
	text "AZURIA"
	para "Baignée par une"
	line "aura d'un bleu"
	cont "mystérieux"
	done

_CeruleanCityTrainerTipsText::
	text "ASTUCE"

	para "Appuyer sur B"
	line "pendant une évo-"
	cont "lution l'annule."
	done

_CeruleanCityBikeShopSign::
	text "Plus vite que le"
	line "vent, plus vite"
	cont "dans les villes!"
	cont "...CYCLES A GOGO"
	done

_CeruleanCityGymSign::
	text "AZURIA"
	line "ARENE #MON"
	cont "CHAMPION: ONDINE"

	para "La petite pest..."
	line "Heu...sirène!"
	done

_OakBattleStartText::
	text "Bonjour, <PLAYER>!"

	para "Comment va ton"
	line "#DEX?"

	para "Que fais-je ici?"

	para "Je m'entraîne,"
	line "bien sûr!"

	para "Je viens ici"
	line "une fois par"
	cont "semaine pour"
	cont "m'entraîner depuis"
	cont "que j'avais ton"
	cont "âge!"

	para "À propos de ce"
	line "troisième #MON"
	cont "de départ:"

	para "Je l'ai élevé"
	line "depuis que toi et"
	cont "<RIVAL> êtes"
	cont "partis."

	para "Il est devenu"
	line "un excellent"
	cont "combattant!"

	para "Je vais te montrer"
	line "comment on se"
	cont "battait avec les"
	cont "#MON de mon"
	cont "temps!"
	done

_OakBattleWinText::
	text "R-"
	line "Remarquable!"
	prompt

_OakBattleLoseText::
	text "On dirait que"
	line "j'ai encore"
	cont "le niveau!"
	prompt

_OakBeatenText::
	text "Le travail d'un"
	line "dresseur n'est"
	cont "jamais vraiment"
	cont "terminé!"

	para "Toi et tes"
	line "#MON devez"
	cont "toujours viser"
	cont "à vous améliorer!"

	para "Viens t'entraîner"
	line "avec moi parfois."

	para "J'ai de super"
	line "histoires à"
	cont "raconter!"
	done

_OakFirstLoseText::
	text "Super combat!"
	para "Comme on pouvait"
	line "s'y attendre du"
	cont "CHAMPION!"
	para "Je pense que je"
	line "devrais te"
	cont "récompenser."
	para "Ah, et si je"
	line "faisais ça?"
	para "Prends cette note"
	line "au labo de"
	cont "l'ILE CRAMOISIE."
	para "Mon collègue"
	line "y travaille!"
	para "Il saura quoi"
	line "faire!"
	para "..."
	para "..."
	para "..."
	para "Le PROF. CHEN a"
	line "écrit une note"
	cont "pour toi!@"
	text_end